...
이후 `run_pipeline.py / main()`의 단계 번호를 따라 각 함수를 찾아가면 됩니다.
줄 번호는 수정할 때 바뀌므로 아래에서는 **파일과 함수 이름**으로 연결합니다.

## 1. 머신러닝에서 “학습한다”는 것은 실제 코드에서 무엇을 의미하나요?

`src/train.py / train_model()`에서 입력 이미지의 복원 오차를 계산하고,
...
`src/model.py`는 어떤 계산을 할지 정하고, `train.py`는 계산에 사용되는 숫자를 바꿉니다.
...
`src/dataset.py / PartDataset`은 “몇 장인가?”와 “i번째 이미지를 어떻게 읽나?”를 담당합니다.
...
`src/inference.py / anomaly_scores()`는 `mean(dim=(1,2,3))`으로
...
`run_pipeline.py`는 test inference 전에 threshold를 결정합니다.
...
실제 이번 기본 실행에서는 구멍 누락 25개를 모두 놓쳤습니다.
`sample_reconstruction.png`의 구멍 누락 예시에서도 모델은 누락된 상태를 복원합니다.
...
`src/threshold.py / choose_threshold()`는 정상 validation score의 95 percentile을 구합니다.
...
기본 실행의 고정값 0.01은 모든 test 이미지를 정상으로 판정해 불량 100개를 놓쳤습니다.
...
이번 기본 실행은 TP 64, TN 94, FP 6, FN 36입니다.
...
`generate_dataset.py / draw_part()`는 원래 정상에도 촬영 편차를 넣습니다.
`evaluate.py / run_variation_experiments()`는 test normal에 추가 편차를 주고
...
`inference.py / benchmark_inference()`는 batch size 1로 warm-up 10회 후 50회 반복해
